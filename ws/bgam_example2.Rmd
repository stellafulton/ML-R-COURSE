...
# back to the moorhens! temporal patterns in moorhen populations - so temporal autocorrelation of the data is something we will have to thin about 
...
# with GAMS - we often fit bayesian models because we can't fit frequentist & because you want to frame conclusions in a different way BUT for GAMS bayesian isn't as evolved

# NB - there were missing values in our dataset (some years they didn't count Moorhens - so put reed.brmsP$data$MoorhenKauai to call from the model rather than the original dataset
```
...
```{r negative binomial, cache=TRUE}
