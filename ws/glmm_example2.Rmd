...
norin_lmer <- lmer(CHANGE~TRIAL*SMR_contr+MASS+(1|FISHID),data=norin,REML=FALSE)
...
norin_lmer1 <- lmer(CHANGE~TRIAL*SMR_contr+offset(MASS)+(1|FISHID),data=norin,REML=FALSE)
...
norin_lmer2 <- lmer(CHANGE~TRIAL+SMR_contr+MASS+(1|FISHID),data=norin,REML=FALSE)

# MULTIPLICATIVE MODEL (interaction) without mass 
norin_lmer3 <- lmer(CHANGE~TRIAL*SMR_contr+(1|FISHID),data=norin,REML=FALSE)

```
# Compare multiplicative vs. additive models with & without mass
```{r AIC values}
AICc(norin_lmer,norin_lmer1,norin_lmer2,norin_lmer3)
...
norin_lmer3a <- update(norin_lmer3,REML=TRUE)

# at the moment it is a random intercept model because each fish is its own control
```

```{r random intercept & slope model }
norin_lmer3b <- update(norin_lmer3a,~TRIAL*SMR_contr+(SMR_contr|FISHID)) # different slope per fish # model failed to converge - won't work- this is too complex i.e. each fish has a different response to metabolic rate # different resposne for each trial for each different fish
norin_lmer3c <- update(norin_lmer3a,~TRIAL*SMR_contr+(TRIAL|FISHID)) 
...
plot_model(norin_lmer3a, type='diag')[-2] %>% plot_grid
# this all looks pretty good
```

# Model investigation / hypothesis testing
```{r}
summary(norin_lmer3a)
